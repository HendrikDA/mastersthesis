...
\begin{itemize}
	...
	\item \textbf{MCP Backend}: Within the mcp-backend folder of the project, simply duplicate the \texttt{.env.example} file and rename the duplicated file to \texttt{.env}. Ensure it is in the root folder of the ./mcp-backend/ folder. Within the \texttt{.env} file, you will have to  set the value of the key named \textbf{OPENAI\_API\_KEY}. You may have to generate a key within OpenAI's API platform. All other values may remain as they are for the prototype.
\end{itemize}
...
If you do not have access to the above files, please reach out to the author or advisors of this thesis.
...
\textbf{Windows}: If you are on Windows, this step may require you to use WSL (Windows Subsystem for Linux) and to set the HOME variable via \texttt{SET HOME=\textbackslash{}C:\textbackslash{}Users\textbackslash{}<username>}. It may be required to execute these commands from a normal terminal instead of the Windows PowerShell.
