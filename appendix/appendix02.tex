...
The first is to download the application code for \prototype{} from GitHub: \href{https://github.com/HendrikDA/masuta-ea-chatbot-prototype}{https://github.com/HendrikDA/masuta-ea-chatbot-prototype}

The second prerequisite is to download Docker and Docker Compose. You can find instructions on how to set this up on your machine under this link: \href{https://docs.docker.com/compose/install/}{https://docs.docker.com/compose/install/}.
...
\begin{itemize}
	\item \textbf{Project Root Folder}: Within the root folder of the project, simply duplicate the \textit{.env.example} file and rename the duplicated file to \textit{.env}. Ensure it is in the root folder of the project. No values in the .env file need to be changed for the prototype to work.
	\item \textbf{Frontend}: Within the frontend folder, simply duplicate the \textit{.env.example} file and rename the duplicated file to \textit{.env}. Ensure it is in the root folder of the ./frontend/ folder. No values in the .env file need to be changed for the prototype to work.
	\item \textbf{MCP Backend}: Within the mcp-backend folder of the project, simply duplicate the \textit{.env.example} file and rename the duplicated file to \textit{.env}. Ensure it is in the root folder of the ./mcp-backend/ folder. Within the .env file, you will have to  set the value of the key named \textbf{OPENAI\_API\_KEY}. You may have to generate a key within OpenAI's API platform. All other values may remain as they are for the prototype.
\end{itemize}
...
\begin{lstlisting}[language=Docker, label={lst:first_time_setup}]
...
If at any time you would like to hard-reset the entire application, you may run the following commands:
% Resetting the docker containers
\begin{lstlisting}[language=Docker, label={lst:reset_docker}]
docker compose down
rm -rf $HOME/neo4j/data
rm -rf $HOME/neo4j_empty/data
\end{lstlisting}

After resetting the application, you will have to restart with the first time setup.
...
% Run Masuta
\begin{lstlisting}[language=Docker, label={lst:start_masuta}]
docker compose up
\end{lstlisting}
...
If Docker does not automatically open the frontend in your browser, navigate to it under \href{http://localhost:3000/}{http://localhost:3000/}. You are now ready to start using \prototype.
