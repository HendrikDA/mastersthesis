...
\caption{A screenshot of a subsection of the textbook's knowledge graph. It shows how different node types (e.g. documents and sections) are connected within their own sub-graphs. On the right hand side is the meta-data for the selected node. Notice the "text" label within the meta-data where it is clear to see the actual contents of the textbook that derived this node.}
\label{fig:textbook_kg}
\end{figure}


% Todo: describe how the rest of the speedparcel files were then parsed.
Building on top of this, the individual parsers for the business object model, business capability support matrix, and cross-application data-flow diagrams were built. Each of these are an adaptation of the original textbook parser and work in a similar manner.


This implementation method quickly reached its limits as the imported datasets became more and more complex. 


Todo: This source \cite{mihindukulasooriya2023text2kgbench} talks about how natural language text can be used to \textbf{create} the knowledge graph. Def use this when explaining how i created my speedparcel imports.

%%% XML Transpiler
\subsubsection{Creating the XML Transpiler}
\label{sub:xml_transpiler}
...
%Todo: Explain XPATH. https://en.wikipedia.org/wiki/XPath
%Todo: Explain APOC Load XML: https://neo4j.com/labs/apoc/4.2/import/xml/
