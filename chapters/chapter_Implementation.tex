...
The previous chapters answer the question of how things were iteratively built, why they were built as such, and how the final system was reviewed. This chapter contains all information on what the finished \prototype{} prototype is, including its architecture, individual components, features, data processing mechanisms, and a sequence diagram of how data flows through the application. Code snippets of critical parts of the final prototype can be seen in the appendix's chapter \ref{appendix:ch:code_snippets}.
...
This section describes each component in more detail with the goal of clarifying how and why the individual components are implemented the way they are.
...
Tuning these system prompts greatly alters the quality of results of the entire system. While the second system prompt is rather straightforward, the first system prompt for incoming user prompts is the result of fine-tuning during development in all four action research cycles, as described in section \ref{sec:action_research_cycles}. With less information in this system prompt, the generated Cypher's ability to retrieve the requested information worsens.
...
This covers all functionality in the backend. It is the central part of the prototype through which all data flows and all application logic is handled. The parsing and conversion of data is described in section \ref{sec:filling_the_databases}.
...
With this method, the entire textbook can be queried via Cypher. The result is a knowledge graph where nodes that belong to the same section of the textbook are connected within their own sub-graph.  A visualization of this can be seen in figure \ref{fig:textbook_kg}.
