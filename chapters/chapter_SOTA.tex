...
By applying the PRISMA framework as mentioned above, it was possible to reduce the amount of sources from an initial count of \textit{to do} sources down to \textit{to do} sources after the first step and down to the final \textit{to do} sources after the final step.	
...
Author Jung (2019) \cite{jung2019purpose} also conducted eight interviews in which enterprise architects, consultants, and decision makers in the German logistics industry to identify concerns that  enterprise architecture management is expected to address in practice. The results highlight that stakeholders deal with recurring topics such as providing information about applications, optimizing application landscapes, and reducing the number of applications to save costs. These concerns form a practical starting point for examining how enterprise architecture data can be structured, queried, and analyzed in order to support questions about existing landscapes.
...
These categories will be relevant again later, as these are also applicable within the domain of enterprise architecture management. Each question posed to the system within this research can be broken down into one of these four categories.
...
Laurenzi et al. (2024) \cite{laurenzi2024llm} address this challenge by proposing a six-step process for creating a knowledge graph with the assistance of LLMs. The six-step cycle is broken down as follows:
\begin{enumerate}
	...
	\item Add the data to the knowledge graph.
